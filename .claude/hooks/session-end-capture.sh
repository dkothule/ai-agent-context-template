#!/usr/bin/env bash
# AI Context — Claude Code session-end capture (SessionEnd)
#
# Repairs a promise shipped since v0.4.x: "a session log exists when the session
# ends". The retired reminder asked the agent to write one, but Claude Code discards
...
const lines = ['---', 'autosaved: true', 'source: claude', 'capture: session-end',
...
lines.push(`local_transcript_ref: ${ref || 'unknown'}`, '---', '',
  '# Session-end autosave (Claude Code)', '',
