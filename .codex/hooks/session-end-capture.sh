#!/usr/bin/env bash
# AI Context — Codex session-end capture (SessionEnd)
#
# Repairs a promise shipped since v0.4.x: "a session log exists when the session
# ends". The retired reminder asked the agent to write one, but Codex discards
...
const lines = ['---', 'autosaved: true', 'source: codex', 'capture: session-end',
